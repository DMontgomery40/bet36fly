...
function referenceWindow(protocol?: Experiment['protocol']) {
  const end = protocol?.plasticity_onset_ms, width = protocol?.dan_baseline_window_ms;
  if (typeof end !== 'number' || !Number.isFinite(end) || typeof width !== 'number' || !Number.isFinite(width) || width <= 0 || width > end) return null;
  const stimulus = protocol?.stimulus_ms;
  const cueKnown = typeof stimulus === 'number' && Number.isFinite(stimulus) && stimulus >= 0;
  return { start: end - width, end, cueKnown, insideCue: cueKnown && end <= stimulus };
}

function DopamineReference({ protocol }: { protocol?: Experiment['protocol'] }) {
  const window = referenceWindow(protocol);
  const mode = protocol?.dan_reference ?? ([2, 3].includes(protocol?.schema_version ?? 0) ? 'tonic-baseline' : undefined);
  return <>
    {typeof protocol?.plasticity_onset_ms === 'number' && Number.isFinite(protocol.plasticity_onset_ms) && <p><strong>{mode === 'none' ? 'Raw dopamine drive (schema 4):' : mode === 'tonic-baseline' ? 'Phasic dopamine drive:' : 'Dopamine reference:'}</strong> plasticity and its eligibility traces start {decimal(protocol.plasticity_onset_ms, 0)} ms into each trial; {mode === 'none'
      ? <>both rule terms use the raw compartment-mean DAN spike count, an event-based approximation to the upstream Jiang and Litwin-Kumar rate rule. Each compartment's DAN reference rate over the preceding {decimal(protocol.dan_baseline_window_ms, 0)} ms is measured and reported, not subtracted.</>
      : mode === 'tonic-baseline'
        ? <>each compartment's DAN reference rate over the preceding {decimal(protocol.dan_baseline_window_ms, 0)} ms is subtracted from the DAN signal in both rule terms. This signed baseline subtraction can change gains without teaching, including when activity falls below the reference after stimulus offset. It does not establish cue-specific learning.</>
        : <>Reference subtraction mode is not recorded.</>}</p>}
    {window && <p className="fine">Recorded DAN reference window: {decimal(window.start, 0)}–{decimal(window.end, 0)} ms. {window.insideCue ? 'This window is during the cue, not a resting tonic measurement.' : window.cueKnown ? 'This window extends beyond the recorded cue; it is not a resting tonic measurement.' : 'Cue timing is not recorded; this reference is not a resting tonic measurement.'} Gate rates average calibration trials; arm rates average training trials.</p>}
  </>;
}
...
function RewardJobDetail({ experiment, job }: { experiment: Experiment; job: ExperimentJob }) {
  ...
  return <section className="reward-run-detail" aria-label="Selected reward arm details"><div className="section-heading"><h3>{job.variant}</h3><span className="reward-status">{statusLabel(job.status)}</span></div>
    ...
    {evidence ? <><h4>Measured gain and circuit evidence</h4><dl className="reward-evidence">
      ...
      <div><dt>KC active fraction</dt><dd>{percent(evidence.kc_active_fraction)}</dd></div><div><dt>DAN spikes</dt><dd>{number(evidence.dan_spikes)}</dd></div><div><dt>MBON mean rate</dt><dd>{evidence.mbon_mean_hz === undefined ? '—' : `${decimal(evidence.mbon_mean_hz, 2)} Hz`}</dd></div><div><dt>{referenceWindow(experiment.protocol)?.insideCue ? 'Cue-window DAN reference rate' : 'DAN reference rate'} (home / away)</dt><dd>{hertzPair(evidence.tonic_dan_hz)}</dd></div>
...
export function RewardExperimentsView({ data, error, selectedExperimentId, selectedJobId, onSelectExperiment, onSelectJob }: RewardExperimentsViewProps) {
  ...
  const mask = reward?.anatomy?.plasticity_mask;
