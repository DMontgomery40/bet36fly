...
const knownStatus = new Set(['pending', 'registered', 'running', 'complete', 'completed', 'queued', 'incomplete', 'failed', 'gate_failed', 'not_run_gate_failed', 'cancelled', 'budget_stopped']);
...
function Conditioning({ experiments, diagnostics, error }: { experiments: ExperimentIndex | null; diagnostics: RewardDiagnosticIndex | null; error: string }) {
  ...
  return <section className="conditioning-evidence" aria-label="Conditioning and reversal"><h2>Conditioning and reversal</h2>
    ...
    {runs.map(run => <article key={run.id}><h3>{run.id} · {status(run.status)}</h3>
      <p className="warning-text">Conditioning scientific verdict: {run.conditioning_validation?.validation_status === 'validated' ? run.conditioning_validation.evidence_status : 'unverified'}. {run.conditioning_validation ? `Artifact validation: ${run.conditioning_validation.validation_status}; ${run.conditioning_validation.validated_calls} calls checked.` : 'No conditioning artifact validation is available.'}</p>
      {run.conditioning_validation?.error && <p role="alert">{run.conditioning_validation.error}</p>}
      {run.conditioning_validation?.note && <p>{run.conditioning_validation.note}</p>}
      {run.conditioning_validation?.validation_status === 'validated' && <dl>{Object.entries(run.conditioning_validation.stages).map(([name, verdict]) => <div key={name}><dt>{label(name)}</dt><dd>Recomputed {verdict.all_passed ? 'passed' : 'failed'}{verdict.classification ? ` · ${label(verdict.classification)}` : ''}</dd></div>)}</dl>}
      ...
      <p>Calls returned / completed / ambiguous intent: {run.conditioning?.calls?.returned ?? 'unavailable'} / {run.conditioning?.calls?.completed ?? 'unavailable'} / {run.conditioning?.calls?.ambiguous ?? 'unavailable'}.</p>
      <p>Prerequisite panels: {run.conditioning?.prerequisite_run_ids?.join(', ') || 'unavailable'}.</p>
      <div className="table-scroll"><table><caption>Required conditioning stages</caption><thead><tr><th>Stage</th><th>Execution</th><th>Progress</th><th>Reason / missing evidence</th></tr></thead><tbody>{stages.map(id => { const canonical = ({ acquisition_ab: 'acquisition-primary', acquisition_cd: 'acquisition-challenge', reversal_ab: 'reversal' } as Record<string, string>)[id]; const job = run.jobs.find(row => row.id === id || row.id === canonical); const stage = run.conditioning?.stages?.find(row => row.id === id || row.id === canonical); return <tr key={id}><th scope="row">{label(id)}</th><td>{status(stage?.status || job?.status)}</td><td>{job?.total != null ? `${job.completed ?? 'unavailable'} / ${job.total}` : 'unavailable'}</td><td>{stage?.status_reason || job?.error || job?.cancellation_reason || (stage?.missing_criteria?.length ? stage.missing_criteria.join(', ') : 'Criteria coverage has not been validated.')}</td></tr>; })}</tbody></table></div>
