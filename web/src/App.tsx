...
import type { Brain, Game, Games, Inference, Ledger, Source, Sport, Status, Training } from './types';

type Tab = 'observatory' | 'training' | 'ledger';
function initialTab(): Tab { const hash = window.location.hash.slice(1); return hash === 'training' || hash === 'ledger' ? hash : 'observatory'; }
...
export default function App() {
  ...
  useEffect(() => {
    if (status.data?.refresh.status === 'complete' || status.data?.refresh.status === 'failed') { void games.reload(); void ledger.reload(); }
  }, [status.data?.refresh.status, games.reload, ledger.reload]);
  async function refresh() {
    setRefreshRequested(true); setActionError('');
    try { await request('/api/refresh', { method: 'POST' }); await status.reload(); await Promise.all([games.reload(), ledger.reload()]); }
  ...
  async function predict(game: Game) {
    ...
    try { const result = await request<Inference>(`/api/predict/${encodeURIComponent(game.id)}`, { method: 'POST' }); setInference(result); if (window.innerWidth <= 720) requestAnimationFrame(() => document.querySelector('.brain-section')?.scrollIntoView({ behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'instant' : 'smooth', block: 'start' })); await Promise.all([games.reload(), ledger.reload()]); }
  ...
  return <><a className="skip-link" href="#content">Skip to content</a><header className="header"><a className="brand" href="#observatory" aria-label="BET36FLY observatory">BET<span>36</span>FLY</a><div className="brand-description">Fruit fly connectome<br/>sports experiment</div><nav aria-label="Main navigation">{([{ id: 'observatory', name: 'Observatory' }, { id: 'training', name: 'Training' }, { id: 'ledger', name: 'Pick ledger' }] as const).map(item => <a key={item.id} href={`#${item.id}`} aria-current={tab === item.id ? 'page' : undefined}>{item.name}</a>)}</nav><div className="header-actions"><span className="paper"><i/>Paper mode</span><button className="refresh" disabled={refreshing} onClick={() => void refresh()}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 6v5h-5M20 11a8 8 0 1 0-1 6M20 6l-2 2"/></svg>{refreshing ? 'Refreshing…' : 'Refresh games'}</button></div><time className="updated" dateTime={status.data?.updated_at}>{status.data?.updated_at ? date(status.data.updated_at) : 'Connecting…'}</time></header>
    <main id="content">
      ...
      {tab === 'observatory' ? <div className="observatory"><div className="science"><div className="hero"><h1>A small brain. A new game.</h1><p>Real fly wiring. Real games. Paper picks.</p></div><BrainView brain={brain.data} error={brain.error} inference={inference} busy={!!busyId} retry={() => void brain.reload()}/><dl className="stats"><div><dt>Neurons</dt><dd>{number(status.data?.brain?.neurons)}</dd></div><div><dt>Connections</dt><dd>{number(status.data?.brain?.edges)}</dd></div><div><dt>Training games</dt><dd>{number(report?.splits?.train?.n ?? training.data?.progress?.splits?.train?.n ?? status.data?.training?.splits?.train?.n)}</dd></div><div><dt>Runtime</dt><dd>{status.data?.runtime || '—'}</dd></div></dl><Curve curve={report?.curve || training.data?.progress.curve || []}/>{!report && <TrainingProgress progress={training.data?.progress || status.data?.training}/>}</div><GamesView data={games.data} error={games.error} loading={games.loading} sport={sport} onSport={setSport} onPredict={game => void predict(game)} busyId={busyId} modelReady={modelReady} selectedId={inference?.game.id}/></div> : tab === 'training' ? <TrainingView data={training.data} error={training.error} loading={training.loading}/> : <LedgerView data={ledger.data} error={ledger.error} loading={ledger.loading}/>}
