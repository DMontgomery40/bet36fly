...
export type Page = 'overview' | 'backtest' | 'methods' | 'learning';
...
export function route(hash: string): Page { return hash === '#backtest' || hash.startsWith('#backtest/') ? 'backtest' : hash === '#methods' ? 'methods' : hash === '#learning' ? 'learning' : 'overview'; }
