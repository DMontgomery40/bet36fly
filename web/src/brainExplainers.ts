...
export const EXPLAINERS: Record<string, BrainExplainer> = {
  sample: {
    ...
    paragraphs: [
      ...
      'Coordinates locate somas, the cell bodies. Lines connect their projected positions so adjacency can be inspected; they do not trace axons, dendrites or the microscopic locations of individual contacts. Two dots that overlap in one projection need not be close in three dimensions. Rotating, zooming and panning move the camera and nothing else, and the neuron selector provides the same access without precise pointing.',
