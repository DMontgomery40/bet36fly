"""Six matched DPM interventions with immutable artifacts and no plasticity."""
...
from bet36fly.sensory_diagnostic import dpm_weights  # noqa: E402
...
def run(path):
    ...
    weights = counts * np.repeat(signs.astype(np.float32), np.diff(ptr)) * np.float32(.275)
    ...
    try:
        for mode in config['modes']:
            engine = SensoryEngine(ptr, post, dpm_weights(nodes, ptr, weights, mode), sensory)
            for trial in config['trials']:
                reference = np.load(base / f'trial-{trial:02d}.npz')
