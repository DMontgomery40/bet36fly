...
(async () => {
 ...
 for(const wanted of ['Activity gate · passed','Sensory identity code (glomerular-tuning-v1)','Cell-type input gains','labeled lines','GABAergic APL neurons','Feature-to-glomerulus map','PAM12','MBON09','Fixed class-frequency prior','Cue-window reference rate','Evoked above unpulsed probe','same cue and seed','not a resting tonic measurement']) assert.ok(passedText.includes(wanted),`passed-gate view lacks: ${wanted}`);
 assert.ok(!passedText.includes('Evoked above tonic'));
 ...
 for(const wanted of ['Activity gate · failed','Phasic dopamine drive','Declared gate margins','KC set overlap between calibration games','KCs still firing after stimulus offset','Cue-window reference rate','Scheduled forced spikes','Evoked above unpulsed probe','same cue and seed','not a resting tonic measurement','not game-specific']) assert.ok(gateText.includes(wanted),`failed-gate view lacks: ${wanted}`);
 assert.ok(!gateText.includes('Sensory identity code'),'schema-2 run must not show a schema-3 encoder');
 assert.ok(!gateText.includes('Evoked above tonic'));
