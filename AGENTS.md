...
- **Associative learning stage (September 13 evening):** David authorized implementing and demonstrating dopamine-dependent learning in the actual circuit and testing its predictive contribution. Its contract is [EXPERIMENT_ASSOCIATIVE.md](docs/EXPERIMENT_ASSOCIATIVE.md); read the [associative handoff](docs/ASSOCIATIVE_LEARNING_HANDOFF.md) before continuing it. New engine, configs and identities (`associative-*`); never report the four questions (mechanism implemented / conditioning qualified / plasticity improves held-out prediction / application uses a learned checkpoint) as one badge. Conditioning-01 failed and stays failed; conditioning-02 passed its predeclared criteria; the reserved 2018 block may be fetched only for a frozen, committed candidate and never rerun.
