...
def make_circuit(root, protocol, *, n_features=16):
    ...
    engine = RewardEngine(ptr, post, weights, arrays['sensory'], arrays['kc'], np.asarray(dans),
                          ...
                          learning_rule=protocol.get('learning_rule', 'event'),
                          rate_tau_ms=protocol.get('rate_tau_ms', 100.),
                          plastic_mask=mask)
    anatomy = dict(neurons=len(ids), kc=len(arrays['kc']), dans=len(dans), plastic_edges=len(edges),
                   dan_reference=engine.dan_reference, learning_rule=engine.learning_rule,
                   rate_tau_ms=engine.rate_tau_ms if engine.learning_rule == 'rate-bridge-v1' else None,
                   plasticity_mask=mask_audit,
