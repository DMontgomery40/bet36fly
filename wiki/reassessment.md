...
## External fast-weight navigation proposal — September 14

Peter Wang's public MaleCNS analysis nominates hΔH, hΔA, hΔI and hΔG as sites where a journey vector might be stored in rapidly updated synaptic strengths. The source repository, its corrective audit and its reduced simulations were inspected at commit `3e6a266`; the corrected eight-bin route simulation reproduced exactly. The proposal is anatomically motivated and experimentally useful, but it is not a measured plasticity mechanism: the update is imposed, candidate dopamine/octopamine contacts are not simulated as causal gates, activity storage remains possible, and competing output routes remain unresolved. [Community verdict](community.md#fast-weight-navigation-candidate--inspected-and-partly-reproduced-september-14-2026) · [evidence record](../docs/evidence/community-sources-2026-09-14/fast-weight-navigation.md).

Nothing in the frozen sensory result, associative conditioning result, product model pointer or current handoff changes. A central-complex synaptic-state integrator is now a more concrete option under [research direction B](research-directions.md#b-preserve-temporal-state-deliberately), with a required non-sports write/read/reset assay and a new experiment identity before any prediction test.
