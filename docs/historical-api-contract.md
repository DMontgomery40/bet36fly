# Historical API and retired product contract

Retained for source and endpoint compatibility. These screens and polling flows are no longer mounted in the product. The current [sensory API contract](api-contract.md) is authoritative for the application.
...
## Conditioning evidence in GET /api/experiments and GET /api/experiments/{experiment_id}

`kind: dopamine-conditioning` manifests are displayed separately from historical sports. Optional `conditioning` metadata carries configured/recorded rule and mask, document hashes, prerequisite IDs, attempted/returned/completed/ambiguous-intent counts, wall budget, reasons and stages. Production stage IDs are `acquisition-primary`, `acquisition-challenge`, and `reversal`; historical presentation IDs remain supported. Stored metadata is sanitized before rendering.

Each conditioning manifest receives an independently computed `conditioning_validation`: `validation_status` (`validated|invalid|stored-only`), `evidence_status` (`passed|failed|incomplete|unverified`), nullable `all_passed`, `validated_calls`, per-stage recomputed verdicts, explanatory `note`, and nullable `error`. A stored pass flag or generic job completion cannot populate a validated pass. The reader verifies the actual original/second qualification pair, current approved source/data/document bindings, exact ordered call matrix, canonical/probe/replay gain ancestry, frozen and masked bytes, integer cue-window responses, fixed KC partitions, and selected complete numerical replays. Count-domain and exact float32 gain arithmetic preserve fixed threefold boundaries without an added tolerance.

All calls retain complete numerical fingerprints and compact scientific arrays. The 32 selected original calls and their 32 replays retain complete numerical outputs. Unselected raw traces cannot be reconstructed from fingerprints; responses are recomputed from retained cue-window counts. Bound evidence combines final per-edge checks with separately tested native electrical and analytic-tail group observations, not reconstructed transient per-edge paths. Interrupted or partial evidence cannot pass. A durable invocation journal distinguishes a persisted intent of unknown outcome from a known invocation/return; no interrupted identity is resumed.

The offline `scripts/run_reward_conditioning.py --pair-id <id>` entry point refuses failed, incomplete, mixed, stale or unverified qualification before engine construction. It fixes 1632 planned calls, a 1640 hard ceiling, and 1200 wall seconds. There is no browser control or API action that launches conditioning. Current mechanism failure continues to block actual conditioning. Artifact links use only the registry allowlist; creating this harness is software verification, not demonstrated learning.
