...
def fixture_files(tmp_path, corruption=None):
    ...
    with atlas.open("w", newline="") as stream:
        ...
        writer.writeheader()
        writer.writerows(dict(body_id=100+j, graph_index=j, type="PPL101" if j < 2 else "PAM12", instance=str(j)) for j in range(24))
    for i in range(32):
        events = np.zeros((2000, 24), np.int32)
        mass = np.zeros((2000, 24), np.float64)
        events[499, 0] = events[500, 0] = 1
        mass[499, 0], mass[500, 0] = 1., 1.+i/100
        bodies = np.arange(100, 124, dtype=np.int64)
        if i == 31:
            if corruption == "cell_order": bodies[[0, 1]] = bodies[[1, 0]]
            elif corruption == "silent_mass": mass[10, 3] = 1.
            elif corruption == "nonfinite": mass[500, 0] = np.nan
            elif corruption == "missing_trial": continue
        np.savez_compressed(result/f"case_{i:02d}.npz", map__body_ids=bodies,
                            map__dan_columns=np.arange(24, dtype=np.int32),
                            map__dan_compartments=np.array([0, 0]+[1]*22, np.int32),
                            raw_dan_events=events, per_cell_release=mass,
...
def test_full32_per_cell_counts_units_and_quiet_cells(tmp_path):
    ...
    assert (first["minimum_event_mass"], first["maximum_event_mass"]) == (1., 1.31)
    assert quiet["events"] == 0 and quiet["minimum_event_mass"] is None and quiet["maximum_event_mass"] is None
    ...
    with pytest.raises(FileExistsError): summarize(result, atlas, tmp_path/"summary")
