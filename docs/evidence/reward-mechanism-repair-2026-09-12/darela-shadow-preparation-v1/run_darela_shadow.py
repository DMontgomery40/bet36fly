...
import run_dan_causal_prefix_v2 as reader  # noqa: E402
import weight_state_guard_reference as guard_ref  # noqa: E402
from darela_release import release_events, WT1_P, WT1_TAU_S  # noqa: E402
from darela_release_bridge_replay import replay, FIELDS  # noqa: E402
...
def validate_saved(out, plan, guard=lambda: None):
    ...
    require(
        0 <= state["wall_seconds"] < 1200 and 0 <= completion["elapsed_through_summary"] < 1200,
        "terminal cap",
    )
    require(state["invocations"] == state["returned"] == dict(release=32, bridge=32), "helper counts")
    require(
        identity["plan"] == plan
    ...
    for i, meta in enumerate(plan["expected_rows"]):
        ...
        require(detail["index"] == i and reader.same_json(detail["metadata"], meta), "saved row identity")
        ...
        require(detail["gain_bound_observations"] == int(v["bound_counts"].sum()), "saved bound count")
        rows.append(
            dict(
                metadata=meta,
                gains=v["gains"],
    ...
    require(
        summary["gain_bound_observations"] == sum(r["gain_bound_observations"] for r in rows),
        "saved bound total",
    )
    expected_screen = (
    ...
    require(
        ...
        and summary.get("native_calls") == 0
