...
def summarize(result, atlas_path, destination):
    ...
    for path in paths:
        with np.load(path, allow_pickle=False) as z:
            ...
            if bodies is None:
                ...
                stats = [dict(events=0, pre_onset_events=0, admitted_events=0,
                              release_mass=0., admitted_release_mass=0.,
                              minimum_event_mass=None, maximum_event_mass=None,
                              endpoint_min=[float("inf")]*3,
                              endpoint_max=[float("-inf")]*3) for _ in bodies]
            ...
            for j, cell in enumerate(stats):
                ...
                if emitted.size:
                    ...
                    cell["minimum_event_mass"] = min(lo, cell["minimum_event_mass"]) if cell["minimum_event_mass"] is not None else lo
                    cell["maximum_event_mass"] = max(hi, cell["maximum_event_mass"]) if cell["maximum_event_mass"] is not None else hi
    ...
    for body, comp, cell in zip(bodies, compartments, stats, strict=True):
        ...
        records.append(dict(body_id=int(body), graph_index=int(identity["graph_index"]),
                            type=identity["type"], instance=identity["instance"],
                            channel="home" if comp == 0 else "away", trials=32, **cell))
    assert bindings == {path: sha(Path(path)) for path in bindings}
    destination.mkdir()
    report = dict(scope="Descriptive aggregation of32 saved untaught histories. Dimensionless engineered release mass, not concentration; pooled plasticity cannot establish an individual DAN's causal contribution.",
                  bindings=bindings, cells=records)
