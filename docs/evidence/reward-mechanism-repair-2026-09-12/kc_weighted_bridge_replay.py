"""Pure fixed-contract replay of nonnegative fractional KC/DAN impulses.

This is the existing antisymmetric bridge with weighted KC events, not a new
cellular model. No native library, graph loader, RNG or captured history is
...
def _inputs(kc, dan, pk, pc, mask, groups, initial, learning):
    ...
    if (
        ...
        or any(not np.isfinite(x).all() or np.any(x < 0) or np.any(x > 1) for x in (kc, dan))
    ):
        raise ValueError("Expected matching finite [0,1] impulse matrices, with steps > 500.")
...
def replay(kc, dan_means, pk, pc, mask, groups, *, initial=None, learning=True):
    ...
    Inputs must be bounded event masses, not rates. DAN columns are already
