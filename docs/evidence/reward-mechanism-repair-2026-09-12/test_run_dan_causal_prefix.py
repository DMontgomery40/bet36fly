...
spec = importlib.util.spec_from_file_location("prefix_driver", HERE / "run_dan_causal_prefix.py")
...
def synthetic_archive_study(tmp_path, monkeypatch):
    ...
    sampled = np.concatenate(
        [np.arange(7, 13, dtype=np.int32), maps["dan_indices"], maps["kc_indices"], maps["sensory_indices"]]
    )
