...
@pytest.mark.parametrize(
    "file,key,value",
    [
        ("parent-result.json", "exit_code", False),
        ("parent-result.json", "exit_code", 0.0),
        ("parent-result.json", "wall_seconds_cap", 1200.0),
        ("parent-result.json", "wall_seconds", False),
        ("summary.json", "native_calls", False),
        ("summary.json", "network_calls", 0.0),
    ],
)
def test_producer_terminal_numeric_alias_family(tmp_path, row_fixture, file, key, value):
    import json
    import audit_darela_shadow as a

    pp, source, _, bound = synthetic_archive(tmp_path, row_fixture)
    target = (source.parent if file == "parent-result.json" else source) / file
    obj = json.loads(target.read_text())
    obj[key] = value
    target.write_text(json.dumps(obj))
    completion = json.loads((source / "completion.json").read_text())
    completion["bindings"] = [
        bound(source / n) for n in ["identity.json", "status.json", "summary.json", "attempts.jsonl"]
    ]
    (source / "completion.json").write_text(json.dumps(completion))
    parent = json.loads((source.parent / "parent-result.json").read_text())
    parent["child_completion"] = bound(source / "completion.json")
    (source.parent / "parent-result.json").write_text(json.dumps(parent))
    paths = list(source.iterdir()) + [source.parent / "parent-result.json"]
    with pytest.raises(ValueError):
        a.validate_terminals(
            json.loads(pp.read_text()), pp.read_bytes(), source, a.Store([bound(p) for p in paths])
        )


@pytest.fixture(scope="module")
def completed_audit_fixture(tmp_path_factory, row_fixture):
    import audit_darela_shadow as a

    base = tmp_path_factory.mktemp("completed-independent-synthetic")
    pp, source, loader, _ = synthetic_archive(base, row_fixture)
    out = base / "audit"
    result = a.audit(pp, source, out, _source_loader=loader, _check_runtime=False)
...
def mutated_audit_fixture(tmp_path, original, mutation):
    import json
    import shutil
    import audit_darela_shadow as a
    ...
    for i, row in enumerate(report["rows"]):
        row["reference_artifact"] = a.binding(out / f"reference_{i:02d}.npz")
    mutation(report)
    for i, row in enumerate(report["rows"]):
        a.write_json(out / f"row_{i:02d}.json", row)
    for name in ["audit.json", "status.json"]:
        a.write_json(out / name, report)
    completion = json.loads((out / "completion.json").read_text())
    completion["audit"] = a.binding(out / "audit.json")
    completion["bindings"] = [a.binding(out / Path(b["path"]).name) for b in completion["bindings"]]
    a.write_json(out / "completion.json", completion)
    return out, report
...
def test_parent_rehashed_classification_and_type_family(tmp_path, completed_audit_fixture, kind):
    import launch_audit_darela_shadow as parent

    def mutate(r):
        g = next(iter(r["candidate_guards"].values()))
        if kind == "failed_row":
            r["rows"][31]["status"] = "failed_numerical_comparison"
        elif kind == "unknown_row":
            r["rows"][31]["status"] = "unreviewed"
        elif kind == "guard_flag":
            g["passed"] = False
        elif kind == "guard_fail":
            g.update(ticks=[1] * 8, passed=False)
        elif kind == "missing_guard":
            r["candidate_guards"].pop(next(iter(r["candidate_guards"])))
        elif kind == "extra_guard":
            r["original_guards"]["extra"] = dict(g)
        elif kind == "bound_total":
            r["gain_bound_observations"] = 1
        elif kind == "bound_row":
            r["rows"][0]["bound_contacts"] = 1
            r["gain_bound_observations"] = 1
        elif kind == "bound_bool":
            r["rows"][0]["bound_contacts"] = False
        elif kind == "attempted_float":
            r["attempted"] = 32.0
        elif kind == "wall_bool":
            r["wall_seconds"] = False
        elif kind == "scope_bool":
            r["native_calls"] = False
        elif kind == "scope_float":
            r["source_solver_calls"] = 0.0
        else:
            r["wall_seconds_cap"] = 600.0

    out, r = mutated_audit_fixture(tmp_path, completed_audit_fixture, mutate)
    with pytest.raises(ValueError):
        parent.validate_result(out, r["run_id"])


@pytest.mark.parametrize("kind", ["unresolved", "rejected_bounds", "passed"])
def test_parent_classification_valid_controls(tmp_path, completed_audit_fixture, kind):
    import launch_audit_darela_shadow as parent

    def mutate(r):
        if kind == "unresolved":
            r["rows"][0]["status"] = "rounding_ambiguous"
            r.update(numerical_status="unresolved", screen="invalid_or_unresolved")
        elif kind == "rejected_bounds":
            r["rows"][0]["bound_contacts"] = 1
            r.update(gain_bound_observations=1, screen="rejected_fixed_hypothesis")

    out, r = mutated_audit_fixture(tmp_path, completed_audit_fixture, mutate)
    assert parent.validate_result(out, r["run_id"]) == r


@pytest.mark.parametrize("kind", ["reference", "completion", "terminal_error"])
def test_parent_late_artifact_snapshot_family(tmp_path, completed_audit_fixture, monkeypatch, kind):
    import audit_darela_shadow as a
    import launch_audit_darela_shadow as parent

    out, r = mutated_audit_fixture(tmp_path, completed_audit_fixture, lambda _: None)
    real = a.Store.json

    def read(self, path):
        value = real(self, path)
        if Path(path).name == "row_31.json":
            if kind == "reference":
                (out / "reference_00.npz").write_bytes(b"changed after initial verify")
            elif kind == "completion":
                (out / "completion.json").write_text("{}")
            else:
                (out / "terminal-error.json").write_text("{}")
        return value

    monkeypatch.setattr(a.Store, "json", read)
    with pytest.raises(ValueError):
        parent.validate_result(out, r["run_id"])
