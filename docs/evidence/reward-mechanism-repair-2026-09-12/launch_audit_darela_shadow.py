...
def validate_result(directory, run_id, guard=lambda: None):
    ...
    initial = {
        name: audit.binding(directory / name, guard)
        for name in ["completion.json", "audit.json", "status.json"]
    }

    def read(name):
        return audit.reader.strict_json(
            audit.checked_bytes(directory / name, initial[name], 32 * 1024 * 1024)
        )
    ...
    if (
        ...
        or not audit.reader.same_json(report, status)
        or any(type(r["completed"]) is not int or r["completed"] != 32 for r in [completed, report, status])
        or type(report["attempted"]) is not int
        or report["attempted"] != 32
        or type(report["wall_seconds"]) not in [int, float]
        or not 0 <= report["wall_seconds"] < 600
        or type(completed["elapsed_seconds"]) not in [int, float]
        or not 0 <= completed["elapsed_seconds"] < 600
        or type(report["wall_seconds_cap"]) is not int
        or report["wall_seconds_cap"] != 600
        or any(type(report[k]) is not int or report[k] != 0 for k in ["native_calls", "source_solver_calls"])
    ):
    ...
    for i, row in enumerate(report["rows"]):
        ...
        if (
            not audit.reader.same_json(row, saved)
            or type(row["index"]) is not int
            or row["index"] != i
            or not audit.reader.same_json(row["metadata"], audit.reader.expected_rows()[i])
    ...
    for row in report["rows"]:
        if row["status"] not in known or type(row["bound_contacts"]) is not int or row["bound_contacts"] < 0:
            raise ValueError("Known numerical row status and nonnegative bound count")
    expected_guards = {
        f"{rid}/{noise}/{channel}"
        for rid in audit.reader.RUNS
        for noise in ["base", "alt"]
        for channel in ["home", "away"]
    }
    for name in ["candidate_guards", "original_guards"]:
        matrix = report[name]
        if type(matrix) is not dict or set(matrix) != expected_guards:
            raise ValueError("All eight guard cells required")
        for value in matrix.values():
            ticks = value["ticks"]
            if type(ticks) is not list or len(ticks) != 8 or any(type(t) is not int for t in ticks):
                raise ValueError("Exact eight integer guard totals")
            exact_pass = 9 * sum(ticks) ** 2 <= 16 * sum(t * t for t in ticks)
            if type(value["passed"]) is not bool or value["passed"] is not exact_pass:
                raise ValueError("Guard pass must match exact integer arithmetic")
    contacts = sum(row["bound_contacts"] for row in report["rows"])
    if type(report["gain_bound_observations"]) is not int or report["gain_bound_observations"] != contacts:
        raise ValueError("Total bound contacts must equal all32 row counts")
    numerical = "passed" if all(row["status"] == "passed" for row in report["rows"]) else "unresolved"
    screen = (
        "invalid_or_unresolved"
        if numerical != "passed"
        else "passed_necessary_untaught_screen"
        if contacts == 0 and all(value["passed"] for value in report["candidate_guards"].values())
        else "rejected_fixed_hypothesis"
    )
    if report["numerical_status"] != numerical or report["screen"] != screen:
        raise ValueError("Overall classification must follow all32 rows, all8 guards and bound totals")
    store.verify()
    if any(audit.binding(directory / name, guard) != item for name, item in initial.items()):
        raise ValueError("Audit terminal snapshot changed during validation")
    if (directory / "terminal-error.json").exists():
        raise ValueError("Late audit terminal error invalidates completion")
    guard()
    return report
