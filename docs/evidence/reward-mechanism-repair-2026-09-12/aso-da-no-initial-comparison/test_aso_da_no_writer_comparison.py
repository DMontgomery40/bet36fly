...
@pytest.mark.parametrize("kc,dan", product((False, True), repeat=2))
...
def test_near_resonance_against_two_independent_high_precision_forms(epsilon):
    ...
    got = _array(writer.advance(writer.SourceState(*initial), seconds, kc_active=False, dan_active=True, params=params))
...
def test_same_current_weight_different_latent_memory_diverges_during_quiet():
    ...
    for initial in (first, second):
        ...
        np.testing.assert_allclose(_array(result), reference_advance(astuple(initial), 600, False, False), atol=1e-12, rtol=1e-12)
