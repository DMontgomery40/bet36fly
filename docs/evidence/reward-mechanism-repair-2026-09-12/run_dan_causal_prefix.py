"""Bounded, output-only audit of saved current-bridge causal prefixes.
...
def _identity_context(plan, store):
    ...
    ids = store.npy(root / "data/brain/ids.npy")
    equal(m["body_ids"], np.asarray(ids[m["sample"]]), "body/global map")
    for role in ["kc", "sensory"]:
        equal(
            m[role + "_indices"],
            np.asarray(store.npy(root / f"data/brain/{role}.npy")).astype(np.int32),
    ...
    require(np.isin(outputs, store.npy(root / "data/brain/mbon.npy")).all(), "MBON retained identity")
    ...
    return m, summaries, np.array(outputs, np.int32), saved
    ...
    try:
        for i, row in enumerate(plan["expected_rows"]):
            ...
            if active != rid:
                ...
                expected = np.concatenate([outputs, m["dan_indices"], m["kc_indices"], m["sensory_indices"]])
